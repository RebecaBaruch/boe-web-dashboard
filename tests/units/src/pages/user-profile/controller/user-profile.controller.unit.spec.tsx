...
jest.mock(
    ...
    () => ({
      ...
      default: ({ activeTab, onTabChange }) => (
        <div>
          ...
          <button onClick={() => onTabChange('farm-profile')}>Farm Profile</button>
          <button onClick={() => onTabChange('change-password')}>Change Password</button>
        </div>
      ),
    })
  );
  ...
  describe('UserProfileController', () => {
    ...
    it('should change the tab when a tab button is clicked', () => {
      ...
      expect(screen.getByText('Current Tab: change-password')).toBeInTheDocument();
    });
